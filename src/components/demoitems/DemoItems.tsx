...
const DemoItems = ({ title, websitePic, delayNum, id, techs }: any) => {
  return (
    ...
    >
      ...
      <Link
        href={"/item/" + id}
        className="h-auto flex flex-col justify-start relative shadow-md dark:shadow-black shadow-zinc-400 hover:bg-white dark:hover:bg-black transition-all items-center lg:aspect-video w-[80vw] lg:w-[35vw] max-w-[90vw] bg-zinc-100 dark:bg-zinc-950 rounded-md"
      >
        ...
        <h3 className="text-3xl text-zinc-900 dark:text-zinc-50 text-center uppercase p-10"> {title}</h3>
        ...
        <div className="flex flex-wrap justify-center items-center pt-2 max-w-[80%]"> 
          {techs.map((techName : string, index: number) => {
            return <TechFlag key={"techName_"+index} name={techName}></TechFlag>
          })}
        </div>
